class Controls
{
    ...
    assignHandlers() {
        ...
        const changeSourceForAllDays = (event) => {
            const target = $(event.target);

            const parentColumn = $(target.parents(".column-container")[0]);

            const value = target.val();

            $(parentColumn.find(".hidden-source")).val(value);

            (new Configuration()).setDirty(true);
        };

        const changeNameForAllDays = (event) => {
            const target = $(event.target);
            ...
            $(parentColumn.find(".hidden-title")).val(value);
        ...
        setTimeout(() => {
            ...
            $(".on-update-change-all-days select").on("change", changeSourceForAllDays);

            $(".on-update-change-all-days .multiday-column-name").on("change", changeNameForAllDays);
        }, 1000);
    }
    ...
    render()
    {
        ...
        const controlTemplatesData = {
            ...
            midnightMeetingPosition     : configuration.settings.midnightMeetingPosition,
